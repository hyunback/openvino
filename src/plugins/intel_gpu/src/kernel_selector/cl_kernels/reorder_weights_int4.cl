...
KERNEL(reorder_weights_int4)(const __global INPUT0_TYPE* input, __global OUTPUT_TYPE* output) {
    ...
    const unsigned o0 = (o / 16) * 16 + (o % 16);
    const unsigned o1 = (o / 16) * 16 + (o % 16) + 16;
    ...
    const uint output_idx = GET_FILTER_OS_IYX_OSV_INDEX(OUTPUT, o, i, 0, 0, 16 / 2); // Calculate offset as osv16 due to packing
    // printf("%d\n", output_idx);
    output[output_idx] = packed_out_channels;
#else
