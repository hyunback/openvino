...
ParamsKey ArgMaxMinKernelTopKBitonic::GetSupportedKey() const {
    ParamsKey k;
...
bool ArgMaxMinKernelTopKBitonic::Validate(const Params& p) const {
    if (!ArgMaxMinKernelBase::Validate(p))
        return false;

    const char* algo = std::getenv(ALGO_ENV);
    if (!algo || std::string(algo) != "B")
        return false;
...
ArgMaxMinKernelBase::DispatchData ArgMaxMinKernelTopKBitonic::SetDefault(const arg_max_min_params& params) const {
    ...
    // One workgroup per operation (output slice)
...
void ArgMaxMinKernelTopKBitonic::GetUpdateDispatchDataFunc(KernelData& kd) const {
    kd.update_dispatch_data_func = [this](const Params& params, KernelData& kd) {
        ...
        const size_t sort_array_size = nextPowerOf2(sort_size);
        const size_t ops_size = getOperationNumber(prim_params);
        const size_t elem_size = prim_params.inputs[0].ElementSize();

        kd.internalBuffers.clear();
        // sort_values_buf
        kd.internalBuffers.push_back(elem_size * sort_array_size * ops_size);
        // sort_indices_buf
        kd.internalBuffers.push_back(4 * sort_array_size * ops_size);
        kd.internalBufferDataType = prim_params.inputs[0].GetDType();
    };
}

JitConstants ArgMaxMinKernelTopKBitonic::GetJitConstants(const arg_max_min_params& params) const {
    ...
    jit.AddConstant(MakeJitConstant("SORT_ARRAY_SIZE", sort_array_size));
...
KernelsData ArgMaxMinKernelTopKBitonic::GetKernelsData(const Params& params) const {
    if (!Validate(params))
        return {};
...
KernelsPriority ArgMaxMinKernelTopKBitonic::GetKernelsPriority(const Params&) const {
