...
ParamsKey ArgMaxMinKernelTopKReduction::GetSupportedKey() const {
    ParamsKey k;
...
bool ArgMaxMinKernelTopKReduction::Validate(const Params& p) const {
    if (!ArgMaxMinKernelBase::Validate(p))
        return false;

    // Only activate when env var is set to "A"
    const char* algo = std::getenv(ALGO_ENV);
    if (!algo || std::string(algo) != "A")
        return false;

    const auto& params = static_cast<const arg_max_min_params&>(p);

    // Only for SORT_BY_VALUE mode where current kernel is slow
    if (params.argMaxMinSortType != ArgMaxMinSortType::VALUE)
        return false;

    // Need at least some elements to sort
...
ArgMaxMinKernelBase::DispatchData ArgMaxMinKernelTopKReduction::SetDefault(const arg_max_min_params& params) const {
...
void ArgMaxMinKernelTopKReduction::GetUpdateDispatchDataFunc(KernelData& kd) const {
    kd.update_dispatch_data_func = [this](const Params& params, KernelData& kd) {
        ...
        // values_buf: VALUES_NUM * ops_size elements
        kd.internalBuffers.push_back(elem_size * sort_size * ops_size);
        // indices_buf: VALUES_NUM * ops_size ints
        kd.internalBuffers.push_back(4 * sort_size * ops_size);
        kd.internalBufferDataType = prim_params.inputs[0].GetDType();
    };
}

JitConstants ArgMaxMinKernelTopKReduction::GetJitConstants(const arg_max_min_params& params) const {
...
KernelsData ArgMaxMinKernelTopKReduction::GetKernelsData(const Params& params) const {
    if (!Validate(params))
        return {};
...
KernelsPriority ArgMaxMinKernelTopKReduction::GetKernelsPriority(const Params&) const {
