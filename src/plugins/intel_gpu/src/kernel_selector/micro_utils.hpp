...
using Setting = gemmstone::microkernel::Setting;
...
inline Package select_gemm_microkernel(GEMMOptions option, HWInformation hw_info, SizeParams sizes, const GEMMProblem &problem,
    ...
    return gemmstone::microkernel::selectGEMM(option, hw_info, sizes, problem, reqs, strategyAdjuster, observer);
}
inline Package select_gemm_microkernel(GEMMOptions option, HWInformation hw_info, SizeParams sizes, const GEMMProblem &problem,
        gemmstone::SelectionObserver *observer) {
    return gemmstone::microkernel::selectGEMM(option, hw_info, sizes, problem, {}, nullptr, observer);
}
